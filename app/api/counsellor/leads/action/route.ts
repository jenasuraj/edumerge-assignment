...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const lead = await db.query<{ assigned_to: number | null }>(
    ...
    if (lead.rows[0].assigned_to !== auth.user.id) {
      return failure("You are not allowed to modify this lead", 403);
    }
    ...
    console.error("Counsellor lead update failed", error);
